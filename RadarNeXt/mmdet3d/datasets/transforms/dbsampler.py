...
def path_format(root, path):
    try:
        with open(os.path.join(root, path), 'r'):  # open path successful
            return (path)
    except FileNotFoundError:  # failed to open path
        revised_path = path[:17] + '/' + path[18:] # kitti_gt_database\398_Car_23.bin -> kitti_gt_database/398_Car_23.bin
        try:
            with open(os.path.join(root, revised_path), 'r'):
                return revised_path
        except FileNotFoundError:
            pass
    raise FileNotFoundError(f'No such file or directory: {path}, {revised_path}')
...
class DataBaseSampler(object):
    ...
    def sample_all(self,
        ...
        if len(sampled) > 0:
            ...
            count = 0
            for info in sampled:
                info['path'] = path_format(self.data_root, info['path'])
