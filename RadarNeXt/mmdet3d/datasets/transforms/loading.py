...
# 检查输入路径格式，如果无法打开，则更改格式；如果更改后仍然无效，则raise FileNotFoundError
def path_format(path):
    try:
        with open(path, 'r'):  # open path successful
            return (path)
    except FileNotFoundError:  # failed to open path
        revised_path = path[:57] + '/' + path[59:] # kitti_gt_database\398_Car_23.bin -> kitti_gt_database/398_Car_23.bin
        try:
            with open(revised_path, 'r'):
                return revised_path
...
class LoadPointsFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # pts_file_path = path_format(pts_file_path)
