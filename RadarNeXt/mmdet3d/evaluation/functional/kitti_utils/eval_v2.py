...
def clean_data_v2(gt_anno, dt_anno, current_class, difficulty, dataset_name):
    if dataset_name == 'VoD':
        CLASS_NAMES = ['car', 'pedestrian', 'cyclist']  # for vod
    elif dataset_name == 'TJ4D':
        CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'truck']  # for tj4d
    else:
        raise ValueError(f'The dataset_name must be one of [\'VoD\', \'TJ4D\'], but a \'{dataset_name}\' is given.')
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    ...
    for i in range(num_gt):
        ...
        if ((gt_anno['occluded'][i] > MAX_OCCLUSION[difficulty])
                # or (gt_anno['truncated'][i] > MAX_TRUNCATION[difficulty])  # jly for tj4d and vod radar evaluation
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, dataset_name):
    ...
    for i in range(len(gt_annos)):
        rets = clean_data_v2(gt_annos[i], dt_annos[i], current_class, difficulty, dataset_name)
...
def kitti_eval_v2(gt_annos,
    ...
    if dataset_name == 'VoD':
        class_to_name = {  # for vod and three classes detection
            0: 'Car',
            1: 'Pedestrian',
            2: 'Cyclist',
            3: 'Van',
...
def kitti_eval_coco_style_v2(gt_annos, dt_annos, current_classes, dataset_name):
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos = do_coco_style_eval(
        gt_annos, dt_annos, current_classes, overlap_ranges, dataset_name, compute_aos)
    for j, curcls in enumerate(current_classes):
        # mAP threshold array: [num_minoverlap, metric, class]
        # mAP result: [num_class, num_diff, num_minoverlap]
