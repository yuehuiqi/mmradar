# for the evaluation on the Radar Point Clouds of VoD, TJ4D, and Dual Radar datasets (Waiting for complementary) 
import tempfile
...
from mmdet3d.evaluation import kitti_eval, kitti_eval_v2
...
class KittiMetricv2(BaseMetric):
    """Kitti evaluation metric.

    Args:
        ann_file (str): Annotation file path.
        dataset_name (str): The indicator of current dataset.
    ...
    def __init__(self,
        ...
        super(KittiMetricv2, self).__init__(
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        ]  # 根据预测结果，result中的各样本排列序号，提取从txt文件中读取出的对应的KITTI格式下的标注真值
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str, ap_dict_ = kitti_eval_v2(
                gt_annos, results_dict[name], classes, self.dataset_name, eval_types=eval_types)
    ...
    def bbox2result_kitti(
        ...
        for idx, pred_dicts in enumerate(
            ...
            if len(box_dict['bbox']) > 0:
                ...
                for box, box_lidar, bbox, score, label in zip(
                        ...
                        label_preds):   # box from box_preds = 相机坐标系下的某个预测3D bbox；box_lidar from box_preds_lidar = LiDAR坐标系下的某个预测3D bbox
                    ...
                    anno['dimensions'].append(box[3:6])  # 相机坐标系的3D bbox的dimensions
                    anno['location'].append(box[:3])     # 相机坐标系的3D bbox的location
                    anno['rotation_y'].append(box[6])    # 相机坐标系的3D bbox的延y轴正方向的yaw角
            ...
            if submission_prefix is not None:
                ...
                # 因为来自保存预测结果的列表，results，的sample_idx为每个样本在当前数据子集(val set或test set)中按顺序排列的编号
                # 这导致以sample_idx作为保存文件名并不会指向与其相对应的标注文件，而是仅以该预测结果的对应样本所在子集中的编号作为名称
                # 可以尝试使用当前的sample_idx变量提取对应的样本所在的文件名（int格式）
                # 或是在vod的evaluator中，以保存的文件名作为样本在子集(val set或test set)中的排序编号，从相应的txt文件中提取实际的文件名
                file_idx_int = self.data_infos[sample_idx]['sample_idx']  # 将预测结果对应的样本由排序编号转为对应样本的时间戳(int)
                if self.dataset_name == 'VoD':
                    curr_file = f'{submission_prefix}/{file_idx_int:05d}.txt' # zrx for vod
                elif self.dataset_name == 'TJ4D':
                    curr_file = f'{submission_prefix}/{file_idx_int:06d}.txt' # jly for dual-radar and tj4d
                else:
                    raise ValueError(f'The dataset_name must be one of [\'VoD\', \'TJ4D\'], but a \'{self.dataset_name}\' is given.')
                    ...
                    for idx in range(len(bbox)):
                        print(
                            ...
                            '{:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f}'.format(
                                ...
                                bbox[idx][3], dims[idx][1], dims[idx][2],  # dims的保存顺序是: 1. h, 2. w, 3.l
                                dims[idx][0], loc[idx][0], loc[idx][1],
                                loc[idx][2], anno['rotation_y'][idx],
                                anno['score'][idx]),
                            file=f)  # 此处输出的预测结果，格式与原始标注中的真值格式完全一致
        ...
        return det_annos  # 经过convert_valid_bboxes变换后，预测结果的格式与从pkl中读取出的原始标注的格式一致
                          # 注意：此处的pkl文件中的真值标注的格式是由create_data.py创建后的格式，在evaluator中，真值并未经过Dataset类的变换
    ...
    def bbox2result_kitti2d(
        ...
        if submission_prefix is not None:
            ...
            for i, anno in enumerate(det_annos):
                sample_idx = sample_idx_list[i]
                cur_det_file = f'{submission_prefix}/{sample_idx:06d}.txt'  # jly for tj4d
    ...
    def convert_valid_bboxes(self, box_dict: dict, info: dict) -> dict:
        ...
        if isinstance(box_preds, LiDARInstance3DBoxes):                        
            box_preds_camera = box_preds.convert_to(Box3DMode.CAM,             # 这里也存在一个yaw角的变换，此处默认按照标准的LiDAR到Camera的进行变换
                                                    lidar2cam)                 # 这里没设置correct_yaw=True，那么yaw角也将被变换为在相机坐标系下延y轴的格式
            box_preds_lidar = box_preds
        elif isinstance(box_preds, CameraInstance3DBoxes):
            box_preds_camera = box_preds
            box_preds_lidar = box_preds.convert_to(Box3DMode.LIDAR,            # 这里也存在一个yaw角的变换，此处默认按照标准的Camera到LiDAR的进行变换
                                                   np.linalg.inv(lidar2cam))   # 这里没设置correct_yaw=True，那么yaw角也将被变换为在相机坐标系下延-z轴的格式
        ...
        if valid_inds.sum() > 0:
            return dict(
                ...
                box3d_camera=box_preds_camera[valid_inds].numpy(),  # 相机坐标系下的预测3D bbox
                box3d_lidar=box_preds_lidar[valid_inds].numpy(),    # LiDAR坐标系下的预测3D bbox
