...
@unique
class DualRadarBox3DMode(IntEnum):
    ...
    def convert(
        ...
        src: 'DualRadarBox3DMode',
        dst: 'DualRadarBox3DMode',
        ...
        Args:
            box (Sequence[float] or np.ndarray or Tensor or
                ...
                array/tensor.
            src (:obj:`DualRadarBox3DMode`): The source box mode.
            dst (:obj:`DualRadarBox3DMode`): The target box mode.
        ...
        if single_box:
            assert len(box) >= 7, (
                'DualRadarBox3DMode.convert takes either a k-tuple/list or '
        ...
        if with_yaw:
            yaw = arr[..., 6:7]
        if src == DualRadarBox3DMode.LIDAR and dst == DualRadarBox3DMode.CAM:
            if rt_mat is None:
            ...
            if with_yaw:
                if correct_yaw:
                    ...
                    yaw = -yaw  # 修改：因为Dual Radar的LiDAR坐标系为y轴指向前方，所以不需要在逆时针旋转90°
        elif src == DualRadarBox3DMode.CAM and dst == DualRadarBox3DMode.LIDAR:
            if rt_mat is None:
            ...
            if with_yaw:
                if correct_yaw:
                    ...
                    yaw = -yaw  # 修改：因为Dual Radar的LiDAR坐标系为y轴指向前方，所以不需要在逆时针旋转90°
        elif src == DualRadarBox3DMode.DEPTH and dst == DualRadarBox3DMode.CAM:
            if rt_mat is None:
            ...
            if with_yaw:
                if correct_yaw:
                    ...
                    yaw = -yaw
        elif src == DualRadarBox3DMode.CAM and dst == DualRadarBox3DMode.DEPTH:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[1, 0, 0], [0, 0, 1], [0, -1, 0]])
            xyz_size = torch.cat([x_size, z_size, y_size], dim=-1)
            if with_yaw:
                if correct_yaw:
                    ...
                    yaw = -yaw
        elif src == DualRadarBox3DMode.LIDAR and dst == DualRadarBox3DMode.DEPTH:
            if rt_mat is None:
        ...
        elif src == DualRadarBox3DMode.DEPTH and dst == DualRadarBox3DMode.LIDAR:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[0, 1, 0], [-1, 0, 0], [0, 0, 1]])
            xyz_size = torch.cat([x_size, y_size, z_size], dim=-1)
            if with_yaw:
            ...
            raise NotImplementedError(
                f'Conversion from DualRadarBox3DMode {src} to {dst} '
        ...
        if with_yaw and correct_yaw:
            ...
            if dst == DualRadarBox3DMode.CAM:
                yaw = torch.atan2(-rot_yaw_vector[:, [2]], rot_yaw_vector[:,
                                                                          [0]])
            elif dst in [DualRadarBox3DMode.LIDAR, DualRadarBox3DMode.DEPTH]:
        ...
        if is_numpy:
            ...
            elif dst == DualRadarBox3DMode.LIDAR:
                target_type = LiDARInstance3DBoxes
            elif dst == DualRadarBox3DMode.DEPTH:
