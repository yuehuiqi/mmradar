...
class Radar7PillarFeatureNet(nn.Module):
    def __init__(self,
        ...
        pfn_layers = []
        for i in range(len(feat_channels) - 1): # 0, 1, 2
            in_filters = feat_channels[i]       # 0: in_channels, 1: 64, 2: 128
            out_filters = feat_channels[i + 1]  # 1: 64, 2: 128, 3: 256
            if i < len(feat_channels) - 2:  # 0, 1 < 2
                last_layer = False
            else:                           # i == 2
                last_layer = True 
            pfn_layers.append(
