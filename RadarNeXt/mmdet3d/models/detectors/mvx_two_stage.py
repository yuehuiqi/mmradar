...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def _forward(self, batch_inputs_dict: Dict[List, torch.Tensor],  # by jly for evaluating size and FLOPS
        ...
        img_feats, pts_feats = self.extract_feat(batch_inputs_dict,
                                                 batch_input_metas)
        if pts_feats:
            preds = self.pts_bbox_head(pts_feats)
        if img_feats:
            preds = self.pts_bbox_head(img_feats)
        return preds
