...
class SparseEncoder(nn.Module):
    ...
    def __init__(
            ...
            encoder_paddings: Optional[TwoTupleIntType] = ((1, ), (1, 1, 1),
                                                           (1, 1, 1),
                                                           ((0, 1, 1), 1, 1)),  # for Default, 4xDxHxW inputs, 128x((D-9)/16)x(H/8)x(W/8) outputs
    ...
    def forward(self, voxel_features: Tensor, coors: Tensor,
        ...
        x = self.conv_input(input_sp_tensor)   # 7xDxHxW => SubM(4, 16, 3, 1, 1) => 16xDxHxW 

        encode_features = []
        for encoder_layer in self.encoder_layers:
            x = encoder_layer(x)          # 16xDxHxW => encoder_layer1: subm(16, 16, 3, 1, 1) -> subm(16, 16, 3, 1, 1) -> spconv(16, 32, 3, 2, 1) => 32x(D+1)/2xH/2xW/2
                                          # 32x(D+1)/2xH/2xW/2 => encoder_layer2: subm(32, 32, 3, 1, 1) -> subm(32, 32, 3, 1, 1) -> spconv(32, 64, 3, 2, 1) => 64x(D+3)/4xH/4xW/4
                                          # 64x(D+3)/4xH/4xW/4 => encoder_layer3: subm(64, 64, 3, 1, 1) -> subm(64, 64, 3, 1, 1) -> spconv(64, 128, 3, 2, (0, 1, 1)) => 128x(D-1)/8xH/8xW/8
                                          # 128x(D-1)/8xH/8xW/8 => encoder_layer4: subm(128, 128, 3, 1, 1) -> subm(128, 128, 3, 1, 1) => 128x(D-1)/8xH/8xW/8
            encode_features.append(x)

        # for detection head
        # [200, 176, 5] -> [200, 176, 2]
        out = self.conv_out(encode_features[-1])  # 128x(D-1)/8xH/8xW/8 => spconv(128, 128, (3, 1, 1), (2, 1, 1), 0) => 128x(D-9)/16xH/8xW/8
    ...
    def make_encoder_layers(
        ...
        for i, blocks in enumerate(self.encoder_channels):  # i: 0, blocks: (16, 16, 32); 
                                                            # i: 1, blocks: (32, 32, 64); 
                                                            # i: 2, blocks: (64, 64, 128); 
                                                            # i: 3, blocks: (128, 128)
            blocks_list = []
            for j, out_channels in enumerate(tuple(blocks)):  # i: 0 => j: 0, out_channels = 16; j: 1, out_channels = 16; j: 2, out_channels = 32
                                                              # i: 1 => j: 0, out_channels = 32; j: 1, out_channels = 32; j: 2, out_channels = 64
                                                              # i: 2 => j: 0, out_channels = 64; j: 1, out_channels = 64; j: 2, out_channels = 128
                                                              # i: 3 => j: 0, out_channels = 128; j: 1, out_channels = 128;
                
                padding = tuple(self.encoder_paddings[i])[j]  # i: 0, j: 0 => padding = 1,  i: 0, j: 1 => padding = 1,  i: 0, j: 2 => padding = 1
                                                              # i: 1, j: 0 => padding = 1,  i: 1, j: 1 => padding = 1,  i: 1, j: 2 => padding = 1
                                                              # i: 2, j: 0 => padding = 1,  i: 2, j: 1 => padding = 1,  i: 2, j: 2 => padding = (0, 1, 1)
                                                              # i: 3, j: 0 => padding = 1,  i: 3, j: 1 => padding = 1
                ...
                if i != 0 and j == 0 and block_type == 'conv_module':
                    ...
                    if j == len(blocks) - 1 and i != len(  # i: 0/1/2, len(blocks) = 3; len(self.encoder_channels) = 4
                            self.encoder_channels) - 1:
                        blocks_list.append(          # spconv when j = 2
                            make_block(
                                in_channels,
                                out_channels,
                                3,
                        ...
                        blocks_list.append(         # subm when j != 2 or i = 3
                            SparseBasicBlock(
                                out_channels,
                                out_channels,
                                norm_cfg=norm_cfg,
            ...
            self.encoder_layers.add_module(stage_name, stage_layers)  # encoder_layer1: subm(16, 16, 3, 1, 1) -> subm(16, 16, 3, 1, 1) -> spconv(16, 32, 3, 2, 1)
                                                                      # encoder_layer2: subm(32, 32, 3, 1, 1) -> subm(32, 32, 3, 1, 1) -> spconv(32, 64, 3, 2, 1)
                                                                      # encoder_layer3: subm(64, 64, 3, 1, 1) -> subm(64, 64, 3, 1, 1) -> spconv(64, 128, 3, 2, (0, 1, 1))
                                                                      # encoder_layer4: subm(128, 128, 3, 1, 1) -> subm(128, 128, 3, 1, 1)
        return out_channels
