...
num_epoch = 80
train_cfg = dict(by_epoch=True, max_epochs=num_epoch, val_interval=1, val_begin=1)
val_cfg = dict()
test_cfg = dict()
...
inference_mode = True # Please make sure this parameter is False when you train the network
custom_hooks = [
...
model = dict(
    ...
    middle_encoder=dict(
        in_channels=64, output_shape=[432,432], type='PointPillarsScatter'),
    backbone=dict(
        type='RepDWC',
        in_channels=64,
        layer_nums=[3, 5, 5],
    ...
    bbox_head=dict(
        ...
        tasks=tasks,
        strides=[2, 2, 2, 2],                # one stride for one class, the final predictions from heads are (H/2, W/2) 
        weight=1,
        corner_weight=1,
        iou_weight=1,
        iou_reg_weight=0.5,
        rectifier=[[0.5, 0.5, 0.5, 0.5]],     # weights of predicted scores and ious for each task
