...
class RPN(nn.Module):
    def __init__(self, layer_nums, ds_layer_strides, ds_num_filters, us_layer_strides,
                 us_num_filters, in_channels, norm_cfg=None, **kwargs):
        super().__init__()
...
class RPNV1(nn.Module):
    ...
    def _build_layer(self, inplanes, planes, num_blocks, stride=1):
        ...
        for j in range(num_blocks):
            block.append(nn.Conv2d(planes, planes, 3, padding=1, bias=False))
            block.append(build_norm_layer(self.norm_cfg, planes)[1])
            block.append(nn.ReLU())
...
class RPNV2(nn.Module):
    def __init__(self, layer_nums, in_channels, num_filters, norm_cfg=None):
        super().__init__()
