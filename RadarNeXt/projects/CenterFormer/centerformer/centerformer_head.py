...
class CenterFormerBboxHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 iou_loss=False,
                 corner_loss=False,
                 iou_factor=[1, 1, 4, 1],  # for car, pedestrian, cyclist, and truck
    ...
    def forward(self, x, *kwargs):
        ret_dicts = []      # x: dict = {'hm', 'scores', 'labels', 'order', 'ct_feat', 'mask' 'corner_hm'}
        ...
        return ret_dicts   # ret_dicts: dict = {'hm', 'scores', 'labels', 'order', 'ct_feat', 'mask', 'corner_hm', 'reg', 'height', 'dim', 'rot', 'iou'}

    def _sigmoid(self, x):
        y = torch.clamp(x.sigmoid_(), min=1e-4, max=1 - 1e-4)
        return y

    def loss(self, preds_dicts, example, **kwargs):    # predictions -> preds_dicts; annotations -> example
        losses = {}
    ...
    def post_processing(
        ...
        for i in range(batch_size):
            ...
            if self.use_iou_loss:
                ...
                scores = scores * ious      # new_scores = scores * ious^beta, for beta = [1, 1, 4, 1] corresponding to each class
            ...
            if test_cfg.get('circular_nms', False):
                ...
                # multi class nms
                selected = []
                for c in range(len(test_cfg.nms.nms_iou_threshold)):
                    class_mask = labels == c
                    if class_mask.sum() > 0:
...
def get_box(pred_boxs, order, test_cfg, H, W):  # pred_boxs => (batch_size, box_features, num_obj); H, W => shape of heatmap
    batch = pred_boxs.shape[0]
    obj_num = order.shape[1]
    ys, xs = torch.meshgrid([torch.arange(0, H), torch.arange(0, W)])
    ys = ys.view(1, H, W).repeat(batch, 1, 1).to(pred_boxs)
    xs = xs.view(1, H, W).repeat(batch, 1, 1).to(pred_boxs)   # initialize the voxel_grid by the shape of heatmap

    batch_id = np.indices((batch, obj_num))[0]
    batch_id = torch.from_numpy(batch_id).to(order)
    xs = xs.view(batch, H * W)[batch_id, order].unsqueeze(1) + pred_boxs[:,
                                                                         0:1]  # derive the prediction on x voxel coordinates of {num_obj} of objects
    ys = ys.view(batch, H * W)[batch_id, order].unsqueeze(1) + pred_boxs[:,
                                                                         1:2]  # derive the prediction on y voxel coordinates of {num_obj} of objects

    xs = xs * test_cfg.out_size_factor * test_cfg.voxel_size[                  # derive the prediction on x real-world coordinates of {num_obj} of objects
        0] + test_cfg.pc_range[0]
    ys = ys * test_cfg.out_size_factor * test_cfg.voxel_size[                  # derive the prediction on y real-world coordinates of {num_obj} of objects
...
def get_box_gt(gt_boxs, order, test_cfg, H, W):     # obtain the annotated bboxes from supervised signals to real-world coordinates and sizes 
    ...
    return batch_box_targets  # B M 7; M => num_obj


def get_corresponding_box(x_ind, y_ind, y_mask, y_cls, target_box):  # find corresponding supervised signals based on indices provided by preds_dict['order']
