...
class BaseDecoderRPN(nn.Module):
    ...
    def __init__(
            ...
            hm_head_layer=2,     # following layer building mechanism, for i in range(hm_head_layer - 1), hm_head_layer=2 means there is 1 block before the classification layer
            corner_head_layer=2, # following layer building mechanism, for i in range(corner_head_layer - 1), corner_head_layer=2 means there is 1 block before the classification layer
            corner=False,
        ...
        self._num_filters = ds_num_filters  # [256, 256, 128], [64, 64, 32] instead for pillar_version
        self._layer_nums = layer_nums  # [5, 5, 1]
        self._num_input_features = num_input_features  # 256, 64 instead for pillar_version
        ...
        in_filters = [
            self._num_input_features,  # 256, 64 instead for pillar_version
            self._num_filters[0],      # 256, 64 instead for pillar_version
            self._num_filters[1],      # 256, 64 instead for pillar_version
        ]
        blocks = []

        for i, layer_num in enumerate(self._layer_nums):  # 0: 5, 1: 5, 2: 1
            block, num_out_filters = self._make_layer(
                in_filters[i],                   # 0: 256, 1: 256, 2: 256; 0: 64, 1: 64, 2: 64 instead for pillar_version
                self._num_filters[i],            # 0: 256, 1: 256, 2: 128; 0: 64, 1: 64, 2: 32 instead for pillar_version
                layer_num,                       # 0: 5, 1: 5, 2: 1
                stride=self._layer_strides[i],   # 0: 1, 1: 2, 2: -4
            )
            blocks.append(block)
        self.blocks = nn.ModuleList(blocks)   # x_channels = out_channels[0], x_down_channels = out_channels[1], block[2](x_down)_channels = out_channels[2]
        self.up = nn.Sequential(
            nn.ConvTranspose2d(
                self._num_filters[0],   # 256, 64 instead for pillar_version
                self._num_filters[2],   # 128, 32 instead for pillar_version; x_up_channels = out_channels
                2,
                stride=2,
                bias=False),
        ...
        # center classification head: generate center heatmap supervised by Gaussian heatmaps with full radius.
        hm_head.append(
            nn.Conv2d(
        ...
        if self.corner:
            ...
            # (only for train) corner classification head (CenterNet3D, AFDetV2): generate corner heatmaps supervised by Gaussian heatmaps with half radius
            self.corner_head.append(
                nn.Conv2d(
...
class DeformableDecoderRPN(BaseDecoderRPN):
    ...
    def forward(self, x, batch_data_samples):
        ...
        # FPN
        x = self.blocks[0](x)       # H x W x num_input_features -> H x W x ds_num_filters[0]
        x_down = self.blocks[1](x)  # H x W x ds_num_filters[0] -> H/2 x W/2 x ds_num_filters[1]
        # self.blocks[2](x_down)    # H/2 x W/2 x ds_num_filters[1] -> 2H x 2W x ds_num_filters[2]
        # self.up(x)                # H x W x ds_num_filters[0] -> 2H x 2W x ds_num_filters[2]
        x_up = torch.cat([self.blocks[2](x_down), self.up(x)], dim=1)  # cat(2H x 2W x ds_num_filters[2], 2H x 2W x ds_num_filters[2]) -> 2H x 2W x 2*ds_num_filters[2]

        # heatmap head
        hm = self.hm_head(x_up)  # 2H x 2W x 2*ds_num_filters[2] -> 2H x 2W x 64 -> 2H x 2W x num_classes

        if self.corner and self.corner_head.training:
            corner_hm = self.corner_head(x_up)     # out_channels = 1, but supervised by corner heatmaps, which each of them corresponds to one class 
            corner_hm = self._sigmoid(corner_hm)   # sigmoid to find where the corner keypoints(at the locations where the feature value is 1).

        # find top K center location
        hm = self._sigmoid(hm)  # sigmoid to find where the center keypoints(at the locations where the feature value is 1).
        ...
        if self.training:
            heatmaps, anno_boxes, gt_inds, gt_masks, corner_heatmaps, cat_labels = self.get_targets(  # noqa: E501
                batch_gt_instance_3d)  # derive supervision signals by calling self.get_targets
            batch_targets = dict(
                ind=gt_inds,
                mask=gt_masks,
                hm=heatmaps,
                ...
                cat=cat_labels)
            inds = (gt_inds[0][:, (self.window_size // 2)::self.window_size]).to(labels)    # This sliding (start:end:step) is not active while self.window_size=1 (Sliding Window Algorithm)
            masks = (gt_masks[0][:, (self.window_size // 2)::self.window_size]).to(labels)  # This sliding (start:end:step) is not active while self.window_size=1 (Sliding Window Algorithm)
        ...
        ct_feat = ct_feat[self.batch_id, order]  # B, 500, C    # choose 500 predictions with the highest score as embedding, C = 2*ds_num_filters[2]
        ...
        spatial_shapes = torch.as_tensor(
            [(H, W), (H // 2, W // 2), (H // 4, W // 4)],  # (H, W) -> x_up, (H/2, W/2) -> x, (H/4, W/4) -> x_down
            dtype=torch.long,
        ...
        transformer_out = self.transformer_decoder(
            ct_feat,  # C = 2*ds_num_filters[2]
            self.pos_embedding,
            src,      # C = 2*ds_num_filters[2]
            spatial_shapes,
            level_start_index,
