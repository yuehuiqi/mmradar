...
sgdet_path = '/home/bxk/wsk/SGDet3D/SGDet3D_Source code/SGDet3D'
if sgdet_path in sys.path:
    sys.path.remove(sgdet_path)
...
class VoteSegHead(Base3DDecodeHead):
    ...
    def __init__(self,
        ...
        self.conv_seg = nn.Linear(end_channel, self.num_classes + 1)
    ...
    def losses(self, seg_logit, vote_preds, seg_label, vote_targets, vote_mask):
        ...
        loss = dict()
        weight = torch.ones_like(seg_label, device=seg_logit.device, dtype=seg_logit.dtype)
        num_foreground = sum(seg_label!=self.bg_label)
        num_background = sum(seg_label==self.bg_label)
        weight[seg_label!=self.bg_label] = weight[seg_label!=self.bg_label]*(1-num_foreground/(num_foreground+num_background))
        weight[seg_label==self.bg_label] = weight[seg_label==self.bg_label]*(1-num_background/(num_foreground+num_background))
        loss_sem_seg = self.loss_decode(seg_logit, seg_label, weight)
        loss['loss_sem_seg'] = loss_sem_seg
        ...
        for i in range(bsz):
            ...
            if len(bbox_labels) == 0:
                ...
                if extra_width is not None:
                    enlarged_boxes_all = bboxes.tensor.clone()
                    for c in range(self.num_classes):
                        extra_x, extra_y, extra_z = extra_width[c][0], extra_width[c][1], extra_width[c][2]
                        enlarged_boxes = enlarged_boxes_all[bbox_labels==c].clone()
                        enlarged_boxes[:, 3:4] += extra_x * 2
                        enlarged_boxes[:, 4:5] += extra_y * 2
                        enlarged_boxes[:, 5:6] += extra_z * 2
                        enlarged_boxes[:, 2] -= extra_z
                        enlarged_boxes_all[bbox_labels == c] = enlarged_boxes
                    bboxes = bboxes.new_box(enlarged_boxes_all)
                    # bboxes = bboxes.enlarged_box(extra_width)
                inbox_inds = bboxes.points_in_boxes(points).long()
                #判断每一个点是否在某一个边界框里
                this_label = self.get_point_labels(inbox_inds, bbox_labels)
                #利用是否在边界框里和在的框的标签生成语义标签
    ...
    #labels：形状 [N_total]，所有点的语义标签。
    #vote_targets：形状 [N_total, 3]，所有点的投票目标。
    #vote_mask：形状 [N_total]，所有点的投票掩码。
