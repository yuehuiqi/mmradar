# initialization
custom_imports = dict(imports=['projects.LidarSecond.mmdet3d_plugin'])
...
# model settings
voxel_size = [0.04, 0.04, 0.16]

# model settings
model = dict(
    type='VoxelNet',
    voxel_layer=dict(
        max_num_points=15, # max_points_per_voxel
        point_cloud_range=point_cloud_range,
        voxel_size=voxel_size,
        max_voxels=(150000, 150000)),  # (training, testing) max_voxels
    voxel_encoder=dict(
        type='HardSimpleVFE',
        num_features=4),
    middle_encoder=dict(
        type='VoxelResBackBone8x_Lidar',
        input_channels=4,
        output_channels=256,
        grid_size=[37, 1280, 1280]),
    backbone=dict(
        type='SECOND',
        in_channels=256,
        layer_nums=[3, 5, 5],
...
# pipline settings
train_pipeline = [
    dict(type='LoadPointsFromFile', coord_type='LIDAR', load_dim=4, use_dim=[0,1,2,3], file_client_args=file_client_args),
